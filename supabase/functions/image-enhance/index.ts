...
serve(async (req) => {
  ...
  try {
        ...
        messages: [
          {
            role: "user",
            content: [
              { type: "text", text: prompt || "Enhance this photo: make it sharp, photorealistic, with natural lighting and true-to-life colors." },
    ...
    if (!resp.ok) {
      ...
      console.error("AI enhance error:", resp.status, txt);
      if (resp.status === 429) {
    ...
    // Lovable AI image-capable models return image as a data URL on message.images[0].image_url.url
