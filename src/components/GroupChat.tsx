...
export const GroupChat = ({ onBack }: { onBack: () => void }) => {
  ...
  const fetchGroups = async () => {
    const { data } = await supabase.from("chat_group_members").select("group_id").eq("user_id", user!.id);
    if (data && data.length > 0) {
      const groupIds = data.map((d: any) => d.group_id);
      const { data: groupsData } = await supabase.from("chat_groups").select("*").in("id", groupIds);
  ...
  const createGroup = async () => {
    ...
    const { data, error } = await supabase.from("chat_groups").insert({ name: groupName, created_by: user.id }).select().single();
            ...
            <div key={msg.id} className={`flex ${msg.sender_id === user?.id ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2 ${msg.sender_id === user?.id ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>
                {msg.sender_id !== user?.id && (
                  <p className="text-[10px] font-semibold text-accent mb-0.5">{getSenderName(msg.sender_id)}</p>
                )}
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        {groups.length === 0 && (
          <p className="text-center text-muted-foreground text-sm py-12">No groups yet. Create one!</p>
        )}
        {groups.map((group) => (
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="glass max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <Button className="w-full" onClick={createGroup} disabled={!groupName.trim()}>Create Group</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
