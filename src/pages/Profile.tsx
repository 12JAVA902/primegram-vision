...
import { Loader2, Grid, Settings, Trash2, Play } from "lucide-react";
...
const Profile = () => {
  ...
  const fetchPosts = async () => {
    try {
      const { data, error } = await supabase
        .from("posts")
        .select("id, image_url, video_url, media_type, caption, created_at, user_id")
        .eq("user_id", userId)
  ...
  const fetchSuggested = async () => {
    ...
    try {
      ...
      // Get suggested people (not already followed, not self)
      const { data } = await supabase.from("profiles").select("id, username, avatar_url").neq("id", user.id).limit(20);
      const notFollowed = (data || []).filter((p: any) => !followedIds.has(p.id)).slice(0, 10);
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="mb-8">
          <div className="flex items-start gap-4 sm:gap-8 mb-6">
            {isOwnProfile ? (
              <AvatarUpload userId={userId!} currentAvatarUrl={profile.avatar_url} username={profile.username} onUploadComplete={fetchProfile} />
            ) : (
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-4 mb-4">
                ...
                {isOwnProfile ? (
                  ...
                  <Button onClick={handleFollow} variant={isFollowing ? "outline" : "default"}
                    className={!isFollowing ? "bg-gradient-to-r from-primary via-accent to-[hsl(25,95%,53%)] hover:opacity-90" : ""}>
        ...
        {/* Suggested People Bar */}
        {suggested.length > 0 && (
          <div className="mb-6">
            <h3 className="text-sm font-semibold mb-3 text-muted-foreground">Suggested for you</h3>
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
              {suggested.map((s) => (
                <div key={s.id} className="flex flex-col items-center gap-2 min-w-[80px]">
                  ...
                  <span className="text-[10px] text-muted-foreground truncate max-w-[70px]">{s.username}</span>
                  <Button size="sm" variant={followingIds.has(s.id) ? "outline" : "default"}
                    className="h-7 text-[10px] px-3" onClick={() => handleFollowSuggested(s.id)}>
        ...
        <div className="border-t pt-6">
          ...
          <div className="grid grid-cols-3 gap-1">
            {posts.map((post) => (
              <Card key={post.id} className="aspect-square overflow-hidden cursor-pointer hover:opacity-90 transition-opacity relative group">
                {post.media_type === "video" && post.video_url ? (
                  <div className="relative w-full h-full">
                    <video src={post.video_url} className="w-full h-full object-cover" muted />
                    <Play className="absolute top-2 right-2 h-5 w-5 text-white drop-shadow" />
                  </div>
                ) : (
